...
function Customers() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editId) await apiService.updateCustomer(editId, formData);
      else await apiService.createCustomer(formData);
      setShowModal(false);
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="modal-title">{editId ? 'Edit' : 'Add'} Customer</h2>
